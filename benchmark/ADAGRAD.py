...
class AdaGradOptimizerNumPy:
    def __init__(self, input_size, hidden_size, output_size, lr=0.01, epsilon=1e-8):
        ...
        self.G_w_input = np.zeros_like(self.w_input)
        self.G_w_output = np.zeros_like(self.w_output)
        self.G_b_input = np.zeros_like(self.b_input)
        self.G_b_output = np.zeros_like(self.b_output)
    ...
    def backward_pass_regression(self, x_batch, y_batch, z_output, z_input, z_input_activation, batch_size):
        ...
        # Accumulating squared gradients for all params (YOUR ORIGINAL LOGIC)
        self.G_w_input += dw_input**2
        self.G_w_output += dw_output**2
        self.G_b_input += db_input**2
        self.G_b_output += db_output**2

        # Update weights and biases using AdaGrad (YOUR ORIGINAL LOGIC)
        self.w_input -= self.lr * dw_input / (np.sqrt(self.G_w_input) + self.epsilon)
        self.w_output -= self.lr * dw_output / (np.sqrt(self.G_w_output) + self.epsilon)
        self.b_input -= self.lr * db_input / (np.sqrt(self.G_b_input) + self.epsilon)
        self.b_output -= self.lr * db_output / (np.sqrt(self.G_b_output) + self.epsilon)
    ...
    def backward_pass_classification(self, x_batch, y_batch, z_output, z_input, z_input_activation, batch_size):
        ...
        # Accumulating squared gradients for all params (YOUR ORIGINAL LOGIC)
        self.G_w_input += dw_input.T**2
        self.G_w_output += dw_output.T**2
        self.G_b_input += db_input**2
        self.G_b_output += db_output**2

        # Update weights and biases using AdaGrad (YOUR ORIGINAL LOGIC)
        self.w_input -= self.lr * dw_input.T / (np.sqrt(self.G_w_input) + self.epsilon)
        self.w_output -= self.lr * dw_output.T / (np.sqrt(self.G_w_output) + self.epsilon)
        self.b_input -= self.lr * db_input / (np.sqrt(self.G_b_input) + self.epsilon)
        self.b_output -= self.lr * db_output / (np.sqrt(self.G_b_output) + self.epsilon)
...
def benchmark_california_housing():
    print("=== CALIFORNIA HOUSING REGRESSION BENCHMARK (ADAGRAD) ===")
    ...
    print(f"Hyperparameters: lr={lr}, epsilon={epsilon}, epochs={epochs}, batch_size={batch_size}")
    
    # NumPy Implementation
    print("\n--- NumPy AdaGrad Implementation ---")
    start_time = time.time()
    numpy_model = AdaGradOptimizerNumPy(input_size, hidden_size, output_size, lr, epsilon)
    ...
    # PyTorch Implementation
    print("\n--- PyTorch AdaGrad Implementation ---")
    ...
    optimizer = optim.Adagrad(pytorch_model.parameters(), lr=lr, eps=epsilon)
...
def benchmark_fashion_mnist():
    print("\n=== FASHION-MNIST CLASSIFICATION BENCHMARK (ADAGRAD) ===")
...
def plot_results(regression_results, classification_results):
    ...
    axes[0, 0].plot(numpy_reg_losses, 'b-', label=f'NumPy AdaGrad ({numpy_reg_time:.1f}s)', linewidth=2)
    axes[0, 0].plot(pytorch_reg_losses, 'r-', label=f'PyTorch AdaGrad ({pytorch_reg_time:.1f}s)', linewidth=2)
    axes[0, 0].set_title('California Housing - Regression Loss (AdaGrad)')
    ...
    axes[0, 1].plot(numpy_cls_losses, 'b-', label=f'NumPy AdaGrad ({numpy_cls_time:.1f}s)', linewidth=2)
    axes[0, 1].plot(pytorch_cls_losses, 'r-', label=f'PyTorch AdaGrad ({pytorch_cls_time:.1f}s)', linewidth=2)
    axes[0, 1].set_title('Fashion-MNIST - Classification Loss (AdaGrad)')
    ...
    axes[1, 0].plot(numpy_cls_acc, 'b-', label='NumPy AdaGrad', linewidth=2)
    axes[1, 0].plot(pytorch_cls_acc, 'r-', label='PyTorch AdaGrad', linewidth=2)
    axes[1, 0].set_title('Fashion-MNIST - Classification Accuracy (AdaGrad)')
    ...
    axes[1, 1].set_title('Training Time Comparison (AdaGrad)')
...
if __name__ == "__main__":
    print("AdaGrad Optimizer Benchmark: NumPy vs PyTorch")
    print("=" * 50)
    ...
    # Summary
    print("\n" + "=" * 50)
    print("ADAGRAD BENCHMARK SUMMARY")
    print("=" * 50)
    ...
    print(f"\nAdaGrad-specific observations:")
    print(f"- AdaGrad accumulates squared gradients over time")
    print(f"- Learning rate effectively decreases as training progresses")
    print(f"- Well-suited for sparse data and features")
    print(f"- May converge slower initially but stabilizes well")
