...
class RMSpropOptimizerNumPy:
    def __init__(self, input_size, hidden_size, output_size, lr=0.001, beta=0.9, epsilon=1e-8):
        ...
        # Initialize weights and biases with your exact initialization strategy
        self.w_input = np.random.normal(0, np.sqrt(1/input_size), (hidden_size, input_size))
        self.w_output = np.random.normal(0, np.sqrt(1/hidden_size), (output_size, hidden_size))
        self.b_input = np.zeros(hidden_size)
        self.b_output = np.zeros(output_size)
        
        # Initialize exponential decaying average accumulators
        self.E_w_input = np.zeros_like(self.w_input)
        self.E_w_output = np.zeros_like(self.w_output)
        self.E_b_input = np.zeros_like(self.b_input)
        self.E_b_output = np.zeros_like(self.b_output)
    ...
    def backward_pass_regression(self, x_batch, y_batch, z_output, z_input, z_input_activation, batch_size):
        ...
        # Accumulating exponential decaying average of past squared gradients (YOUR ORIGINAL LOGIC)
        self.E_w_input = self.beta * self.E_w_input + (1 - self.beta) * dw_input**2
        self.E_w_output = self.beta * self.E_w_output + (1 - self.beta) * dw_output**2
        self.E_b_input = self.beta * self.E_b_input + (1 - self.beta) * db_input**2
        self.E_b_output = self.beta * self.E_b_output + (1 - self.beta) * db_output**2

        # Updating params (YOUR ORIGINAL LOGIC)
        self.w_input -= (self.lr / (np.sqrt(self.E_w_input) + self.epsilon)) * dw_input
        self.w_output -= (self.lr / (np.sqrt(self.E_w_output) + self.epsilon)) * dw_output
        self.b_input -= (self.lr / (np.sqrt(self.E_b_input) + self.epsilon)) * db_input
        self.b_output -= (self.lr / (np.sqrt(self.E_b_output) + self.epsilon)) * db_output
    ...
    def backward_pass_classification(self, x_batch, y_batch, z_output, z_input, z_input_activation, batch_size):
        ...
        # Accumulating exponential decaying average of past squared gradients (YOUR ORIGINAL LOGIC)
        self.E_w_input = self.beta * self.E_w_input + (1 - self.beta) * (dw_input.T)**2
        self.E_w_output = self.beta * self.E_w_output + (1 - self.beta) * (dw_output.T)**2
        self.E_b_input = self.beta * self.E_b_input + (1 - self.beta) * db_input**2
        self.E_b_output = self.beta * self.E_b_output + (1 - self.beta) * db_output**2

        # Updating params (YOUR ORIGINAL LOGIC)
        self.w_input -= (self.lr / (np.sqrt(self.E_w_input) + self.epsilon)) * dw_input.T
        self.w_output -= (self.lr / (np.sqrt(self.E_w_output) + self.epsilon)) * dw_output.T
        self.b_input -= (self.lr / (np.sqrt(self.E_b_input) + self.epsilon)) * db_input
        self.b_output -= (self.lr / (np.sqrt(self.E_b_output) + self.epsilon)) * db_output
...
class PyTorchNetLeakyReLU(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        with torch.no_grad():
            self.hidden.weight.normal_(0, np.sqrt(1/input_size))
            self.hidden.bias.zero_()
            self.output.weight.normal_(0, np.sqrt(1/hidden_size))
...
def benchmark_california_housing():
    print("=== CALIFORNIA HOUSING REGRESSION BENCHMARK (RMSPROP) ===")
    ...
    print(f"Hyperparameters: lr={lr}, beta={beta}, epsilon={epsilon}, epochs={epochs}, batch_size={batch_size}")
    
    # NumPy Implementation
    print("\n--- NumPy RMSprop Implementation ---")
    start_time = time.time()
    numpy_model = RMSpropOptimizerNumPy(input_size, hidden_size, output_size, lr, beta, epsilon)
    ...
    # PyTorch Implementation
    print("\n--- PyTorch RMSprop Implementation ---")
    ...
    optimizer = optim.RMSprop(pytorch_model.parameters(), lr=lr, alpha=beta, eps=epsilon)
...
def benchmark_fashion_mnist():
    print("\n=== FASHION-MNIST CLASSIFICATION BENCHMARK (RMSPROP) ===")
...
def plot_results(regression_results, classification_results):
    ...
    axes[0, 0].plot(numpy_reg_losses, 'b-', label=f'NumPy RMSprop ({numpy_reg_time:.1f}s)', linewidth=2)
    axes[0, 0].plot(pytorch_reg_losses, 'r-', label=f'PyTorch RMSprop ({pytorch_reg_time:.1f}s)', linewidth=2)
    axes[0, 0].set_title('California Housing - Regression Loss (RMSprop)')
    ...
    axes[0, 1].plot(numpy_cls_losses, 'b-', label=f'NumPy RMSprop ({numpy_cls_time:.1f}s)', linewidth=2)
    axes[0, 1].plot(pytorch_cls_losses, 'r-', label=f'PyTorch RMSprop ({pytorch_cls_time:.1f}s)', linewidth=2)
    axes[0, 1].set_title('Fashion-MNIST - Classification Loss (RMSprop)')
    ...
    axes[1, 0].plot(numpy_cls_acc, 'b-', label='NumPy RMSprop', linewidth=2)
    axes[1, 0].plot(pytorch_cls_acc, 'r-', label='PyTorch RMSprop', linewidth=2)
    axes[1, 0].set_title('Fashion-MNIST - Classification Accuracy (RMSprop)')
    ...
    axes[1, 1].set_title('Training Time Comparison (RMSprop)')
...
if __name__ == "__main__":
    print("RMSprop Optimizer Benchmark: NumPy vs PyTorch")
    print("=" * 50)
    ...
    # Summary
    print("\n" + "=" * 50)
    print("RMSPROP BENCHMARK SUMMARY")
    print("=" * 50)
    ...
    print(f"\nRMSprop-specific observations:")
    print(f"- RMSprop uses exponential decaying average: E[g²] = β·E[g²] + (1-β)·g²")
    print(f"- Solves AdaGrad's diminishing learning rate problem")
    print(f"- No bias correction needed (unlike Adam)")
    print(f"- Good balance between convergence speed and stability")
    print(f"- Works well with non-stationary objectives and RNNs")
