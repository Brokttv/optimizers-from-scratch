...
class AdamOptimizerNumPy:
    def __init__(self, input_size, hidden_size, output_size, lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-8):
        ...
        # Initialize first moment (momentum) and second moment (adaptive learning rates)
        self.m_w_input = np.zeros_like(self.w_input)
        self.m_w_output = np.zeros_like(self.w_output)
        self.m_b_input = np.zeros_like(self.b_input)
        self.m_b_output = np.zeros_like(self.b_output)
    ...
    def leaky_relu(self, z, alpha=0.001):
        return np.maximum(z, alpha * z)

    def leaky_relu_derivative(self, z, alpha=0.001):
        return np.where(z > 0, 1, alpha)

    def forward_pass(self, x):
        z_input = np.dot(x, self.w_input.T) + self.b_input
        z_input_activation = self.leaky_relu(z_input)
    ...
    def backward_pass_regression(self, x_batch, y_batch, z_output, z_input, z_input_activation, batch_size):
        # Increment time step (YOUR ORIGINAL LOGIC)
        self.t += 1
        ...
        activation_prime = self.leaky_relu_derivative(z_input)
        deltas_input = np.dot(deltas_output, self.w_output) * activation_prime / batch_size
        dw_input = np.dot(deltas_input.T, x_batch) / batch_size
        db_input = np.sum(deltas_input, axis=0) / batch_size

        # Updating first moment (Momentum) - YOUR ORIGINAL LOGIC
        self.m_w_input = self.beta_1 * self.m_w_input + (1 - self.beta_1) * dw_input
        self.m_w_output = self.beta_1 * self.m_w_output + (1 - self.beta_1) * dw_output
        self.m_b_input = self.beta_1 * self.m_b_input + (1 - self.beta_1) * db_input
        self.m_b_output = self.beta_1 * self.m_b_output + (1 - self.beta_1) * db_output

        # Updating second moment (Adaptive learning rates) - YOUR ORIGINAL LOGIC
        self.v_w_input = self.beta_2 * self.v_w_input + (1 - self.beta_2) * dw_input**2
        self.v_w_output = self.beta_2 * self.v_w_output + (1 - self.beta_2) * dw_output**2
        self.v_b_input = self.beta_2 * self.v_b_input + (1 - self.beta_2) * db_input**2
        self.v_b_output = self.beta_2 * self.v_b_output + (1 - self.beta_2) * db_output**2

        # Bias correction - YOUR ORIGINAL LOGIC
        m_w_input_hat = self.m_w_input / (1 - self.beta_1**self.t)
        m_w_output_hat = self.m_w_output / (1 - self.beta_1**self.t)
        m_b_input_hat = self.m_b_input / (1 - self.beta_1**self.t)
        m_b_output_hat = self.m_b_output / (1 - self.beta_1**self.t)

        v_w_input_hat = self.v_w_input / (1 - self.beta_2**self.t)
        v_w_output_hat = self.v_w_output / (1 - self.beta_2**self.t)
        v_b_input_hat = self.v_b_input / (1 - self.beta_2**self.t)
        v_b_output_hat = self.v_b_output / (1 - self.beta_2**self.t)

        # Updating params - YOUR ORIGINAL LOGIC
        self.w_input -= self.lr * m_w_input_hat / (np.sqrt(v_w_input_hat) + self.epsilon)
        self.w_output -= self.lr * m_w_output_hat / (np.sqrt(v_w_output_hat) + self.epsilon)
        self.b_input -= self.lr * m_b_input_hat / (np.sqrt(v_b_input_hat) + self.epsilon)
        self.b_output -= self.lr * m_b_output_hat / (np.sqrt(v_b_output_hat) + self.epsilon)
    ...
    def backward_pass_classification(self, x_batch, y_batch, z_output, z_input, z_input_activation, batch_size):
        ...
        _, e = self.cross_entropy_loss(y_batch, z_output)
        
        dw_output = np.dot(z_input_activation.T, e)
        db_output = np.sum(e, axis=0)

        # Gradients for hidden layer
        activation_prime = self.leaky_relu_derivative(z_input)
        deltas_input = np.dot(e, self.w_output) * activation_prime
        dw_input = np.dot(x_batch.T, deltas_input)
        db_input = np.sum(deltas_input, axis=0)

        # Updating first moment (Momentum) - YOUR ORIGINAL LOGIC
        self.m_w_input = self.beta_1 * self.m_w_input + (1 - self.beta_1) * dw_input.T
        self.m_w_output = self.beta_1 * self.m_w_output + (1 - self.beta_1) * dw_output.T
        self.m_b_input = self.beta_1 * self.m_b_input + (1 - self.beta_1) * db_input
        self.m_b_output = self.beta_1 * self.m_b_output + (1 - self.beta_1) * db_output

        # Updating second moment (Adaptive learning rates) - YOUR ORIGINAL LOGIC
        self.v_w_input = self.beta_2 * self.v_w_input + (1 - self.beta_2) * (dw_input.T)**2
        self.v_w_output = self.beta_2 * self.v_w_output + (1 - self.beta_2) * (dw_output.T)**2
        self.v_b_input = self.beta_2 * self.v_b_input + (1 - self.beta_2) * db_input**2
        self.v_b_output = self.beta_2 * self.v_b_output + (1 - self.beta_2) * db_output**2
...
class PyTorchNetLeakyReLU(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        super(PyTorchNetLeakyReLU, self).__init__()
...
def benchmark_california_housing():
    print("=== CALIFORNIA HOUSING REGRESSION BENCHMARK (ADAM) ===")
    ...
    print(f"Hyperparameters: lr={lr}, beta_1={beta_1}, beta_2={beta_2}, epsilon={epsilon}, epochs={epochs}, batch_size={batch_size}")
    
    # NumPy Implementation
    print("\n--- NumPy Adam Implementation ---")
    start_time = time.time()
    numpy_model = AdamOptimizerNumPy(input_size, hidden_size, output_size, lr, beta_1, beta_2, epsilon)
    ...
    start_time = time.time()
    pytorch_model = PyTorchNetLeakyReLU(input_size, hidden_size, output_size).to(device)
    optimizer = optim.Adam(pytorch_model.parameters(), lr=lr, betas=(beta_1, beta_2), eps=epsilon)
...
def benchmark_fashion_mnist():
    print("\n=== FASHION-MNIST CLASSIFICATION BENCHMARK (ADAM) ===")
...
def plot_results(regression_results, classification_results):
    ...
    axes[0, 0].plot(numpy_reg_losses, 'b-', label=f'NumPy Adam ({numpy_reg_time:.1f}s)', linewidth=2)
    axes[0, 0].plot(pytorch_reg_losses, 'r-', label=f'PyTorch Adam ({pytorch_reg_time:.1f}s)', linewidth=2)
    axes[0, 0].set_title('California Housing - Regression Loss (Adam)')
    ...
    axes[0, 1].plot(numpy_cls_losses, 'b-', label=f'NumPy Adam ({numpy_cls_time:.1f}s)', linewidth=2)
    axes[0, 1].plot(pytorch_cls_losses, 'r-', label=f'PyTorch Adam ({pytorch_cls_time:.1f}s)', linewidth=2)
    axes[0, 1].set_title('Fashion-MNIST - Classification Loss (Adam)')
    ...
    axes[1, 0].plot(numpy_cls_acc, 'b-', label='NumPy Adam', linewidth=2)
    axes[1, 0].plot(pytorch_cls_acc, 'r-', label='PyTorch Adam', linewidth=2)
    axes[1, 0].set_title('Fashion-MNIST - Classification Accuracy (Adam)')
    ...
    axes[1, 1].set_title('Training Time Comparison (Adam)')
...
if __name__ == "__main__":
    print("Adam Optimizer Benchmark: NumPy vs PyTorch")
    print("=" * 50)
    ...
    print(f"\nAdam-specific observations:")
    print(f"- Adam combines momentum (β₁={0.9}) and adaptive learning rates (β₂={0.999})")
    print(f"- Bias correction prevents initial steps from being biased toward zero")
    print(f"- Generally faster convergence than AdaGrad/SGD")
    print(f"- Well-suited for noisy gradients and sparse data")
    print(f"- Time step counter (t) is crucial for bias correction")
